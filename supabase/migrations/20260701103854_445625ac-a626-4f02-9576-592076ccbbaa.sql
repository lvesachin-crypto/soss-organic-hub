...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1
    FROM public.plisio_webhook_events e
    WHERE e.order_id = p_order_id
      AND e.signature_valid = true
      AND lower(COALESCE(e.status, '')) IN ('completed', 'success')
      AND (v_dep.invoice_id IS NULL OR e.invoice_id IS NULL OR e.invoice_id = v_dep.invoice_id)
      AND CASE
        WHEN (e.payload->>'source_amount') ~ '^[0-9]+(\.[0-9]+)?$'
          THEN abs(((e.payload->>'source_amount')::numeric) - v_dep.amount_inr) <= 0.5
        ELSE false
      END
  ) INTO v_signed_event_exists;

  IF NOT v_signed_event_exists THEN
    RETURN json_build_object('credited', false, 'reason', 'missing_valid_signed_plisio_webhook');
  END IF;
