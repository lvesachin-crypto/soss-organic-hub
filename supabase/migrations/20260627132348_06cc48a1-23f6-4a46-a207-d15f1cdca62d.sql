CREATE OR REPLACE FUNCTION public.credit_wallet_zapupi(
  ...
  p_utr text DEFAULT NULL::text,
  p_gateway_response jsonb DEFAULT NULL::jsonb
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Security trigger requires a completed transaction trail BEFORE any wallet credit.
