...
BEGIN
  ...
  IF NOT v_is_admin THEN
    IF v_run.engagement_order_id IS NULL OR NOT EXISTS (
      ...
      WHERE eo.id = v_run.engagement_order_id AND eo.user_id = v_uid
    ) THEN
...
GRANT EXECUTE ON FUNCTION public.reschedule_organic_run(uuid, integer, timestamptz) TO authenticated, service_role;
