...
BEGIN
  ...
  SELECT * INTO v_dep FROM public.plisio_deposits WHERE order_id = p_order_id FOR UPDATE;
  ...
  VALUES (v_dep.user_id, 'deposit', v_credit_usd, v_new_balance, 'completed', 'plisio', p_order_id,
  ...
  UPDATE public.wallets SET balance = v_new_balance,
    total_deposited = trunc(COALESCE(v_deposited,0) + v_credit_usd, 4), updated_at = now()
   WHERE user_id = v_dep.user_id;
  UPDATE public.plisio_deposits SET credited = true, status = 'success', updated_at = now() WHERE id = v_dep.id;
