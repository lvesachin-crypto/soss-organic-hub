...
export default function Wallet() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'deposit': return <ArrowDownLeft className="h-4 w-4" style={{ color: '#3b82f6' }} />;
      case 'order': return <ArrowUpRight className="h-4 w-4" style={{ color: '#ef4444' }} />;
      case 'refund': return <RefreshCw className="h-4 w-4" style={{ color: '#2563eb' }} />;
  ...
  const getIconBg = (type: string) => {
    switch (type) {
      case 'deposit': return 'rgba(59,130,246,.1)';
  ...
  return (
    <DashboardLayout>
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#2563EB' }}>Wallet</h1>
        ...
        <div className="rounded-2xl p-6" style={{ background: 'white', border: '1px solid rgba(0,0,0,.06)', boxShadow: '0 2px 12px rgba(0,0,0,.04)' }}>
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            <h2 className="text-lg font-bold" style={{ color: '#2563EB' }}>Transaction History</h2>
              ...
              {displayTransactions.map((tx) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-medium text-[13px] leading-tight truncate max-w-[260px]" style={{ color: '#2563EB' }}>
                      ...
                      <div className="flex items-center flex-wrap gap-x-2 gap-y-0.5 mt-1">
                        ...
                        <span
                          ...
                          style={{
                            background: tx.status === 'pending' ? 'rgba(245,158,11,.1)' : tx.status === 'completed' ? 'rgba(59,130,246,.1)' : 'rgba(239,68,68,.1)',
                            color: tx.status === 'pending' ? '#f59e0b' : tx.status === 'completed' ? '#3b82f6' : '#ef4444',
                          }}
                        >
                          {tx.status}
                        </span>
